...
class MainWindow(QMainWindow):
    ...
    def makeTable(self, cursor):
        ...
        self.tblStudent.setRowCount(cursor.prefetchrows) #커서에 들어있는 데이터 길이만큼 row
        self.tblStudent.setHorizontalHeaderLabels(['학생번호', '학생이름', '핸드폰', '등록년도'])
    
        # 전달받은 cursor를 반복문으로 테이블위젯에 뿌리는 작업
        for i, (std_id, std_name, std_mobile , std_regyear) in enumerate(cursor):
