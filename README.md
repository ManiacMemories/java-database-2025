...
- DML
    ...
    - UPDATE - 데이터 변경, WHERE 조건을 없이 실행하면 테이블 모든 데이터가 수정됨(주의요망!)
        ...
        [WHERE 조건]
        ```
    - DELETE - 데이터 삭제, WHERE 조건 없이 실행하면
        ```sql
        DELETE FROM 테이블명
        [WHERE 조건];
        ```
...
- VIEW : [뷰쿼리](./day05/sql01_뷰.sql)
    ...
    - 뷰라도 INSERT, UPDATE, DELETE가 가능함. 단일뷰에서만
...
- 서브쿼리 : [서브쿼리](./day05/sql02_서브쿼리.sql)
    ...
    - WHERE절 서브쿼리는 JOIN으로 거의 다 변경가능능
- 시퀀스 : [시퀀스](./day05/sql03_시퀀스.sql)
    ...
    - 없어도 기능에는 차이가 없지만 효율을 위해서 사용
        ...
        [CYCLE] -- 최대증가값까지 도달하면 다시 처음으로 돌아갈 것인지
...
- Stored Procedure와 Function를 만들기위해서 사용
    ...
    - 트리거
        ...
        BEFORE|AFTER INSERT|UPDATE ON 테이블{뷰}이름
        BEGIN
            PL/SQL쿼리
        END;
