...
SELECT s.name, d.dname
 ...
 WHERE s.DEPTNO1 = d.DEPTNO
   AND s.DEPTNO1  = (SELECT s.DEPTNO1
					   FROM STUDENT s
					  WHERE s.NAME = 'Anthony Hopkins'); -- 학생 이름따라 학과번호가 변경 가능
...
-- >ANY 서브쿼리의 최소값보다 메인쿼리 큰값을 조회
-- <ANY 서브쿼리의 최대값보다 메인쿼리 작은값을 조회
-- >ALL 서브쿼리의 최대값보다 메인쿼리 큰값을 조회
-- <ALL 서브쿼리의 최소값보다 메인쿼리 작은값을 조회
...
SELECT e.name, e.empno, e.POSITION, e.PAY, g1.PAYAVG
