...
SELECT SUM(salary) -- scalar value
  FROM employees;
...
-- employees 테이블에서 부서와 직군별 급여총액과 직원수를 출력
...
-- employees 테이블에서 부서와 직군별 급여총액과 직원수를 출력하고 department_id가 30에서 90사이이고, 부서직군별급여총액이 20000달러 이상인 데이터만 출력
SELECT department_id, job_id, sum(salary) AS 부서직군별급여총액
	 , count(*)
  FROM employees
